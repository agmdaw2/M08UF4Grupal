...
<body>
    ...
    <div id='practicas'>
        <?php
            $actividad = $_GET["correccion"];
            pregExamen($actividad);
        ?>
    </div>   

    <script>
        let activ = document.querySelectorAll('.activUsuario');
        for(let i= 0; i<activ.length;i++){
            activ[i].addEventListener("click", function(){
                let id = this.id;
                location.href = "correccion.php?correccion=" + id;
            }); 
        }
    </script>
